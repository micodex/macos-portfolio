  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-center w-fit gap-4 rounded-2xl bg-white/8 backdrop-blur-xl border-white/10 border-2 pb-0 px-4`}
      ...
      >
        {state.apps.map((item, index) => (
          <DockItem
            key={index}
            onClick={() => dispatch({ type: "OPEN", id: item.id })}
